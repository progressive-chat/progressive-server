...
std::string random_string(size_t length) {
  ...
  const size_t alphabet_len = std::strlen(kAlphabet);  // 62
...
nlohmann::json to_canonical_object(const nlohmann::json& value) {
  if (!value.is_object()) {
    throw std::runtime_error("Value must be a JSON object");
  }
  
  // Use dump with sorted keys and no whitespace for canonical form
  return nlohmann::json::parse(value.dump(-1, 0, false, nlohmann::json::error_handler_t::strict));
}
