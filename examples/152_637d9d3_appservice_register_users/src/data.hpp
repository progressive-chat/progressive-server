...
class Data {
 public:
  ...
  // --- NEW in 8f27e61: read receipts ------------------------------------------
  /// Store the latest read receipt for (user, room), replacing the previous
  /// one (upstream readreceipt_update; the key is room+0xff+count+0xff+user).
  void readreceipt_update(const std::string& user_id, const std::string& room_id,
                          const nlohmann::json& event);
  /// Receipts of a room newer than `since` as (user_id, count, event).
  struct Receipt {
    std::string user_id;
    uint64_t count = 0;
    nlohmann::json event;
  };
  std::vector<Receipt> readreceipts_since(const std::string& room_id,
                                          uint64_t since) const;
  /// All rooms a server participates in (upstream Rooms::server_rooms).
  std::vector<std::string> server_rooms(const std::string& server) const;
  /// NEW in 8f27e61: federation EDU events (m.receipt) to send to `server`,
  /// since the last EDU count sent there; updates the stored count.
  std::vector<nlohmann::json> select_edus(const std::string& server);
...
// NEW in 71500b1: get participating servers in a room
  ...
  struct ServerSigningKeys {
      ...
      // NEW in 09157b2: superseded keys are kept (merged on every fetch).
      std::map<std::string, VerifyKey> old_verify_keys;
  };
  ...
  /// Add signing keys for a server to cache (merges into the stored document)
  void add_signing_key(const std::string& server_name, const ServerSigningKeys& keys);

  // --- NEW in 09157b2: failure rate limiters -----------------------------------
  /// Exponential backoff for repeatedly failing event fetches: skip `id`
  /// while (now - last) < min(30s * tries^2, 24h).
  bool should_back_off_event(const std::string& event_id) const;
  void back_off_event(const std::string& event_id);
  /// Same for failed server-signature fetches, keyed by origin+key id.
  bool should_back_off_signature(const std::string& key) const;
  void back_off_signature(const std::string& key);
 ...
 private:
  ...
  // NEW in 09157b2: in-memory failure rate limiters (upstream keeps these in
  // Globals behind RwLocks): id -> (last-failure millis, consecutive tries).
  mutable std::mutex ratelimit_mutex_;
  mutable std::map<std::string, std::pair<uint64_t, uint32_t>> bad_event_ratelimiter_;
  mutable std::map<std::string, std::pair<uint64_t, uint32_t>> bad_signature_ratelimiter_;
  sled::Db db_storage_;
