...
// NEW in b2d55160: optional Data for proxy-aware .well-known fetch.
std::pair<FederationDestination, FederationDestination> find_actual_destination(
    const std::string& destination, Data* data = nullptr);
...
// NEW in b2d55160: optional Data applies the configured proxy.
...
// NEW in 68cc743f: shared auth-chain helper (upstream free fn
// get_auth_chain in server_server.rs). Returns the transitive auth-event
// ids of the starting events (seeds themselves excluded unless referenced),
// skipping missing events with a warning — replacing the hand-rolled
// todo-set walks.
...
// NEW in 67f9592b: /event_auth federation endpoint — auth chain for an event.
...
// NEW in e5c71195: forward federation errors to the client (upstream
// Error::FederationError). A remote Matrix error (errcode/error JSON on a
// non-200 response) is surfaced as "Answer from <server>: <message>" instead
// of a generic local error.
...
// Updated in 98f1480e: auth_cache removed, uses db.pdu_get() directly.
