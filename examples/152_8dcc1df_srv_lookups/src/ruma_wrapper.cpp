...
void respond(httplib::Response& res, const MatrixResult<SyncResponse>& result) {
  ...
  auto joined_room_json = [](const SyncResponse& room) {
    ...
    // NEW in f12fbca: state events (Element needs them for membership changes).
    json state = json::array();
    for (const auto& ev : room.state_events) state.push_back(json::parse(ev));
    return json{
        {"account_data", {{"events", json::array()}}},
        {"ephemeral", {{"events", std::move(ephemeral)}}},
        {"state", {{"events", std::move(state)}}},
        {"summary", json::object()},
  ...
  auto room_is_empty = [](const SyncResponse& room) {
    return room.timeline_events.empty() && room.stripped_state.empty() &&
           room.ephemeral_events.empty() && room.state_events.empty();
  };
