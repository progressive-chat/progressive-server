...
void register_admin_routes(httplib::Server& svr, Context& ctx) {
  ...
  // NEW in 594fe5f: block a single media item (by MXC server + media_id).
  // `and_purge` also removes the media, mirroring Conduit's BlockMedia command.
  svr.Post("/_conduit/admin/block_media",
           [&ctx](const httplib::Request& req, httplib::Response& res) {
             if (!require_admin(ctx, req, res)) return;
             ruma::json body = parse_body(req);
             const std::string media_id = body.value("media_id", "");
             ...
             const bool and_purge = body.value("and_purge", false);
             ctx.data->media_block(server_name, media_id, reason);
             const bool purged = and_purge ? ctx.data->media_remove(server_name, media_id, false)
                                           : false;
             ruma::respond(res,
                           ruma::json{{"blocked", true}, {"purged", purged ? 1 : 0}});
           });

  // NEW in 594fe5f: block every media uploaded by one or more local users.
  svr.Post("/_conduit/admin/block_media_from_users",
           [&ctx](const httplib::Request& req, httplib::Response& res) {
             if (!require_admin(ctx, req, res)) return;
             ruma::json body = parse_body(req);
             if (!body.contains("user_ids") || !body["user_ids"].is_array()) {
             ...
             const std::string reason = body.value("reason", "");
             std::optional<uint64_t> after_secs;
             if (body.contains("after_secs") && body["after_secs"].is_number())
               after_secs = body["after_secs"].get<uint64_t>();
             size_t total = 0;
             for (const auto& u : body["user_ids"]) {
               if (!u.is_string()) continue;
               total += ctx.data->media_block_by_user(ctx.data->hostname(), u.get<std::string>(),
                                                      reason, after_secs);
             }
             ruma::respond(res, ruma::json{{"blocked", total}});
           });

  // NEW in 594fe5f: unblock a single media item.
  svr.Post("/_conduit/admin/unblock_media",
           [&ctx](const httplib::Request& req, httplib::Response& res) {
             if (!require_admin(ctx, req, res)) return;
             ruma::json body = parse_body(req);
             const std::string media_id = body.value("media_id", "");
             ...
             const std::string server_name =
                 body.contains("server_name") ? body["server_name"].get<std::string>()
                                              : ctx.data->hostname();
             const bool ok = ctx.data->media_unblock(server_name, media_id);
             ruma::respond(res, ruma::json{{"unblocked", ok}});
           });

  // NEW in 594fe5f: list all blocked media.
  svr.Get("/_conduit/admin/list_blocked_media",
           [&ctx](const httplib::Request& req, httplib::Response& res) {
             if (!require_admin(ctx, req, res)) return;
             ruma::json list = ruma::json::array();
             for (const auto& m : ctx.data->media_list_blocked()) {
               ruma::json entry;
               entry["server_name"] = m.server_name;
               entry["media_id"] = m.media_id;
               entry["unix_secs"] = m.unix_secs;
               if (m.reason) entry["reason"] = *m.reason;
               if (m.sha256_hex) entry["sha256_hex"] = *m.sha256_hex;
               list.push_back(entry);
             }
             ruma::respond(res, ruma::json{{"blocked_media", list}});
           });

  // NEW in c3fb1b0: trigger the time-based retention cleanup immediately (the
  // periodic thread normally does this at the configured interval).
  svr.Post("/_conduit/admin/purge_time_retention",
           [&ctx](const httplib::Request& req, httplib::Response& res) {
             if (!require_admin(ctx, req, res)) return;
             const size_t purged = ctx.data->media_cleanup_time_retention();
             ruma::respond(res, ruma::json{{"media_purged", purged}});
           });

  // NEW in a189b66: "show media" — fetch a media item's bytes so an admin can
  // view it (Conduit sends it into the admin room as a message; we return the
  // bytes with the stored Content-Type, gated by admin auth).
  auto show_content_disposition = [](const std::string& filename) {
  ...
  svr.Post("/_conduit/admin/show_media",
           [&ctx, &show_content_disposition](const httplib::Request& req,
                                             httplib::Response& res) {
             if (!require_admin(ctx, req, res)) return;
             ruma::json body = parse_body(req);
             const std::string media_id = body.value("media_id", "");
             ...
             const std::string server_name =
                 body.contains("server_name") ? body["server_name"].get<std::string>()
                                              : ctx.data->hostname();
             auto media = ctx.data->media_get(server_name, media_id, true);
             if (!media) {
               ruma::respond(res,
                             ruma::json{{"errcode", "M_NOT_FOUND"},
             ...
             res.set_header("Content-Disposition",
                            media->filename ? show_content_disposition(*media->filename)
                                            : std::string("inline"));
           });

  // NEW in fd16e9c: query all information about one media item.
  svr.Post("/_conduit/admin/query_media",
           [&ctx](const httplib::Request& req, httplib::Response& res) {
             if (!require_admin(ctx, req, res)) return;
             ruma::json body = parse_body(req);
             const std::string media_id = body.value("media_id", "");
             ...
             const std::string server_name =
                 body.contains("server_name") ? body["server_name"].get<std::string>()
                                              : ctx.data->hostname();
             auto q = ctx.data->media_query(server_name, media_id);
             if (!q) {
               ruma::respond(res,
                             ruma::json{{"errcode", "M_NOT_FOUND"},
                                        {"error", "Media not found."}},
             ...
             ruma::json out;
             out["is_blocked"] = q->is_blocked;
             if (q->source_file) {
               ruma::json src;
               if (q->source_file->uploader) src["uploader"] = *q->source_file->uploader;
               src["sha256_hex"] = q->source_file->sha256_hex;
               if (q->source_file->filename) src["filename"] = *q->source_file->filename;
               if (q->source_file->content_type)
                 src["content_type"] = *q->source_file->content_type;
               src["unauthenticated_access_permitted"] =
                   q->source_file->unauthenticated_access_permitted;
               src["is_blocked_via_filehash"] = q->source_file->is_blocked_via_filehash;
               src["creation"] = q->source_file->file_info.creation;
               src["last_access"] = q->source_file->file_info.last_access;
               src["size"] = q->source_file->file_info.size;
               out["source_file"] = src;
             }
             ruma::respond(res, out);
           });

  // NEW in fd16e9c: list stored media with optional filters.
  svr.Post("/_conduit/admin/list_media",
           [&ctx](const httplib::Request& req, httplib::Response& res) {
             if (!require_admin(ctx, req, res)) return;
             ruma::json body = parse_body(req);
             ...
             if (body.contains("content_type") && body["content_type"].is_string())
               content_type = body["content_type"].get<std::string>();
             std::optional<uint64_t> before_ms, after_ms;
             if (body.contains("before_ms") && body["before_ms"].is_number())
               before_ms = body["before_ms"].get<uint64_t>();
             if (body.contains("after_ms") && body["after_ms"].is_number())
               after_ms = body["after_ms"].get<uint64_t>();
             ruma::json list = ruma::json::array();
             for (const auto& m : ctx.data->media_list(server_name, user_id, content_type,
                                                       before_ms, after_ms)) {
               ruma::json e;
               e["server_name"] = m.server_name;
               e["media_id"] = m.media_id;
               if (m.uploader) e["uploader"] = *m.uploader;
               if (m.content_type) e["content_type"] = *m.content_type;
               if (m.filename) e["filename"] = *m.filename;
               e["size"] = m.size;
               e["creation"] = m.creation;
               list.push_back(e);
             }
             ruma::respond(res, ruma::json{{"media", list}});
           });
}
