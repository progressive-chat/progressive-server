...
// NEW in 09157b2: merged per-server signing-key document (upstream
// server_signingkeys tree). verify_keys and old_verify_keys are both
// served; individual rows are never written anymore.
std::map<std::string, std::string> Data::get_signing_keys(
    const std::string& server_name) const {
  std::map<std::string, std::string> out;
  auto raw = db_.server_signingkeys.get(server_name);
  if (!raw) return out;
  try {
    nlohmann::json doc = nlohmann::json::parse(*raw);
    if (doc.contains("verify_keys") && doc["verify_keys"].is_object()) {
      for (auto& [key_id, entry] : doc["verify_keys"].items()) {
        if (entry.is_object() && entry.contains("key") && entry["key"].is_string())
          out[key_id] = entry["key"].get<std::string>();
        else if (entry.is_string())
          out[key_id] = entry.get<std::string>();
      }
    }
    if (doc.contains("old_verify_keys") && doc["old_verify_keys"].is_object()) {
      for (auto& [key_id, entry] : doc["old_verify_keys"].items()) {
        if (out.count(key_id)) continue;
        if (entry.is_object() && entry.contains("key") && entry["key"].is_string())
          out[key_id] = entry["key"].get<std::string>();
        else if (entry.is_string())
          out[key_id] = entry.get<std::string>();
...
void Data::add_signing_key(const std::string& server_name, const ServerSigningKeys& keys) {
  ...
  if (auto raw = db_.server_signingkeys.get(server_name)) {
    try {
  ...
  if (!doc.contains("verify_keys") || !doc["verify_keys"].is_object())
    doc["verify_keys"] = nlohmann::json::object();
  if (!doc.contains("old_verify_keys") || !doc["old_verify_keys"].is_object())
    doc["old_verify_keys"] = nlohmann::json::object();
  for (const auto& [key_id, vk] : keys.verify_keys)
    doc["verify_keys"][key_id] = {{"key", vk.key}};
  for (const auto& [key_id, vk] : keys.old_verify_keys)
    doc["old_verify_keys"][key_id] = {{"key", vk.key}};
  db_.server_signingkeys.insert(server_name, doc.dump());
}

// NEW in 09157b2: exponential backoff for repeated failures (upstream
// RateLimitState = (last-failure time, tries); skip while
// (now - last) < min(30s * tries^2, 24h)).
namespace {
bool back_off_due(const std::map<std::string, std::pair<uint64_t, uint32_t>>& map,
                  const std::string& key) {
  auto it = map.find(key);
  if (it == map.end()) return false;
  const uint64_t elapsed = utils::millis_since_unix_epoch() - it->second.first;
  uint64_t wait_ms = 30000ULL * it->second.second * it->second.second;
  const uint64_t cap_ms = 24ULL * 60 * 60 * 1000;
  if (wait_ms > cap_ms) wait_ms = cap_ms;
  return elapsed < wait_ms;
}

void back_off_bump(std::map<std::string, std::pair<uint64_t, uint32_t>>& map,
                   const std::string& key) {
  auto& slot = map[key];
  slot.first = utils::millis_since_unix_epoch();
  slot.second += 1;
}
}  // namespace

bool Data::should_back_off_event(const std::string& event_id) const {
  std::lock_guard<std::mutex> lock(ratelimit_mutex_);
  return back_off_due(bad_event_ratelimiter_, event_id);
}

void Data::back_off_event(const std::string& event_id) {
  std::lock_guard<std::mutex> lock(ratelimit_mutex_);
  back_off_bump(bad_event_ratelimiter_, event_id);
}

bool Data::should_back_off_signature(const std::string& key) const {
  std::lock_guard<std::mutex> lock(ratelimit_mutex_);
  return back_off_due(bad_signature_ratelimiter_, key);
}

void Data::back_off_signature(const std::string& key) {
  std::lock_guard<std::mutex> lock(ratelimit_mutex_);
  back_off_bump(bad_signature_ratelimiter_, key);
}
