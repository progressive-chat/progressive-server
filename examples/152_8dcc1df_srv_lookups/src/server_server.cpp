...
// NEW in b2d55160: apply the configured arbitrary proxy (upstream
// Config::proxy -> reqwest Proxy) to an httplib client when the proxy
// rules match the destination. cpp-httplib only speaks HTTP proxies, so
// the endpoint is applied as host:port regardless of scheme.
template <typename Client>
void apply_proxy(Data& data, Client& client, const std::string& destination) {
  if (auto proxy = data.proxy_config().proxy_for(destination)) {
...
FederationDestination FederationDestination::literal(const std::string& ip,
  ...
  return FederationDestination{true, ip, p};
}
...
std::string FederationDestination::into_url() const {
  return "https://" + into_uri();
}

std::string FederationDestination::into_uri() const {
  // Literals store a bare numeric port; named values carry ":port" or "".
  if (is_literal && !port.empty()) return host + ":" + port;
  return host + port;
}
...
std::pair<FederationDestination, FederationDestination> find_actual_destination(
    const std::string& destination, Data* data) {
  ...
  } else if (auto well_known = request_well_known(destination_str, data)) {
  ...
  } else if (auto srv = lookup_srv_record(destination_str)) {
    // NEW in 8dcc1df: query SRV when .well-known is missing (fixes #29).
    // hostname stays destination_str, so the Host header keeps the original
    // name exactly like upstream (host = original destination).
    actual = FederationDestination::named(srv->first, ":" + std::to_string(srv->second));
  }
...
// NEW in b2d55160: honors the configured proxy (upstream reqwest client
// carried Config::proxy for every request).
std::optional<std::string> request_well_known(const std::string& destination,
  ...
  if (data) apply_proxy(*data, client, destination);
...
std::optional<nlohmann::json> send_request(
  ...
  auto [actual_dest, host_dest] = find_actual_destination(destination, &data);
  ...
  // NEW in e5c71195: split the "host:port" destination for the client
  // constructor. Previously the combined string was passed as the hostname,
  // which can never resolve — no federation send could ever connect.
  std::string fed_host = actual_destination;
  int fed_port = 8448;
  {
    auto all_digits = [](const std::string& s) {
      return !s.empty() && s.find_first_not_of("0123456789") == std::string::npos;
    };
    if (!fed_host.empty() && fed_host.front() == '[') {
      if (auto e = fed_host.find(']'); e != std::string::npos) {
        std::string rest = fed_host.substr(e + 1);
        fed_host = fed_host.substr(1, e - 1);
        if (rest.size() > 1 && rest.front() == ':' && all_digits(rest.substr(1)))
          fed_port = std::stoi(rest.substr(1));
      }
    } else if (auto c = fed_host.rfind(':'); c != std::string::npos) {
      if (all_digits(fed_host.substr(c + 1))) {
        fed_port = std::stoi(fed_host.substr(c + 1));
        fed_host = fed_host.substr(0, c);
      }
    }
  }
  httplib::SSLClient client(fed_host, fed_port);
  client.enable_server_certificate_verification(false);  // sandbox proxy MITM
  // NEW in b2d55160: route federation sends through the configured proxy.
  apply_proxy(data, client, destination);
  ...
  // NEW in e5c71195: non-200 Matrix errors are still returned (callers check
  // errcode), but logged with origin context like upstream's FederationError.
  if (res->status != 200 && is_remote_error(response_json)) {
    std::cerr << "[warn] Federation error from " << destination << ": "
              << remote_error_message(response_json, destination) << "\n";
  }

  return response_json;
}

// NEW in e5c71195: forward federation errors to the client.
bool is_remote_error(const nlohmann::json& response) {
  return response.is_object() && response.contains("errcode");
}

std::string remote_error_message(const nlohmann::json& response,
                                 const std::string& destination) {
  std::string message = response.value("error", response.value("errcode", std::string("unknown error")));
  return "Answer from " + destination + ": " + message;
}


// NEW in 68cc743f: shared auth-chain helper (upstream get_auth_chain /
// get_auth_chain_recursive in server_server.rs). Iterative with a visited
// set, so adversarial cycles terminate; missing events are skipped with a
// warning exactly like upstream.
std::set<std::string> get_auth_chain(
    Data& db, const std::vector<std::string>& starting_events) {
  std::set<std::string> full_auth_chain;
  std::set<std::string> visited;
  std::vector<std::string> stack = starting_events;
  while (!stack.empty()) {
    std::string id = std::move(stack.back());
    stack.pop_back();
    if (!visited.insert(id).second) continue;
    auto pdu_json = db.pdu_get(id);
    if (!pdu_json.has_value()) {
      std::cerr << "[warn] Could not find pdu mentioned in auth events.\n";
      continue;
    }
    try {
      nlohmann::json pdu = nlohmann::json::parse(*pdu_json);
      if (pdu.contains("auth_events") && pdu["auth_events"].is_array()) {
        for (const auto& auth_event : pdu["auth_events"]) {
          if (auth_event.is_string()) {
            full_auth_chain.insert(auth_event.get<std::string>());
            stack.push_back(auth_event.get<std::string>());
          }
        }
      }
    } catch (...) {
...
nlohmann::json get_room_state_ids(
    ...
    // NEW in 68cc743f: use the shared helper instead of the hand-rolled
    // todo-set walk (upstream seeds it with the state ids, as here).
    std::set<std::string> auth_chain_ids = get_auth_chain(db, pdu_ids);
...
// NEW in 67f9592b: /event_auth federation endpoint.
// GET /_matrix/federation/v1/event_auth/:roomId/:eventId -> {auth_chain} by
// walking auth_events from the seed event (missing links skipped with the
// upstream "Could not find pdu mentioned in auth events" spirit).
nlohmann::json get_event_auth(Data& db, const std::string& room_id,
                              const std::string& event_id) {
    using json = nlohmann::json;
    if (!db.pdu_get(event_id)) {
        json error = json::object();
        error["errcode"] = "M_NOT_FOUND";
        error["error"] = "Event not found.";
        return error;
    ...
    json result = json::object();
    result["auth_chain"] = db.federation_auth_chain(room_id, {event_id});
    return result;
}
...
nlohmann::json get_public_rooms_federation(Data& db, const nlohmann::json& request) {
    ...
    for (const auto& room_id : public_rooms) {
        ...
        // NEW in 77a23f89: case-insensitive generic_search_term over name,
        // topic and canonical_alias (was: case-sensitive name-only).
        if (!generic_search_term.empty() && generic_search_term.is_string()) {
            std::string term = utils::ascii_lower(generic_search_term.get<std::string>());
            bool match = false;
            for (const char* field : {"name", "topic", "canonical_alias"}) {
                if (room_info.contains(field) && room_info[field].is_string() &&
                    utils::icontains(room_info[field].get<std::string>(), term)) {
                    match = true;
                    break;
                }
            }
...
// Updated in 98f1480e: removed auth_cache parameter, uses db.pdu_get() directly
// with a local visited set to avoid duplicate work and infinite recursion.
// 
// Logic:
// 1. Check database (timeline + outlier PDUs via db.pdu_get)
// 2. If in DB, recursively fetch its auth_events
// 3. If not in DB, fetch from origin server over federation via GET /event
// 3a. On success, store locally via pdu_append
// 3b. Recursively fetch auth chain
// 
...
std::vector<nlohmann::json> fetch_and_handle_events(
    ...
    std::unordered_set<std::string> visited;  // Track visited event_ids to avoid cycles/duplicates
    
    // Helper lambda to recursively fetch a single event and its auth chain
    std::function<void(const std::string&)> fetch_event = [&](const std::string& id) {
        if (visited.find(id) != visited.end()) return;
        visited.insert(id);

        // NEW in 09157b2: skip events that recently failed (exponential
        // backoff: 30s * tries^2, capped at 24h).
        if (db.should_back_off_event(id)) {
            std::cerr << "[debug] Backing off from " << id << "\n";
            return;
        }
        
        // a. Look in the main timeline / outlier PDU tree (db.pdu_get checks both)
        std::optional<std::string> pdu_json_opt = db.pdu_get(id);
        if (pdu_json_opt.has_value()) {
            std::cerr << "[debug] Found " << id << " in db\n";
            json pdu = json::parse(*pdu_json_opt);
            
            // If we found it in DB, recursively fetch its auth_events first
            if (pdu.contains("auth_events") && pdu["auth_events"].is_array()) {
        ...
        // b. Ask origin server over federation
        ...
        if (response && response->is_object() && response->contains("pdu")) {
            ...
            // NEW in 09157b2: rate-limit retries of bad events.
            db.back_off_event(id);
        }
    };
...
// NEW in b2d55160: honors the configured proxy.
// NEW in 09157b2: returns the full document (verify + old keys) so the
// caller can persist the merged per-server entry.
std::optional<Data::ServerSigningKeys> fetch_server_keys_live(
    const std::string& origin, Data* data) {
  Data::ServerSigningKeys keys;
  bool any = false;
  ...
  try {
    ...
    if (data) apply_proxy(*data, client, origin);
    if (auto res = client.Get("/_matrix/key/v2/server")) {
      if (res->status == 200) {
        auto doc = nlohmann::json::parse(res->body, nullptr, false);
        if (!doc.is_discarded()) {
          for (const char* section : {"verify_keys", "old_verify_keys"}) {
            if (!doc.contains(section) || !doc[section].is_object()) continue;
            for (auto& [key_id, entry] : doc[section].items()) {
              if (!entry.is_object() || !entry.contains("key") ||
                  !entry["key"].is_string())
                continue;
              Data::VerifyKey vk{entry["key"].get<std::string>()};
              if (std::string(section) == "verify_keys")
                keys.verify_keys[key_id] = vk;
              else
                keys.old_verify_keys[key_id] = vk;
              any = true;
            }
          }
        }
...
std::optional<std::string> verify_federation_request(
  ...
  // Origin's keys: merged per-server cache first, then live fetch
  // (upstream fetch_signing_keys). Fetched keys are persisted via
  // add_signing_key (NEW in 09157b2 — previously they were discarded).
  // NEW in 09157b2: back off from repeatedly failing signature fetches.
  std::map<std::string, std::string> keys = db.get_signing_keys(*origin);
  const std::string backoff_key = *origin + '\xff' + *key_id;
  if (keys.empty() && !db.should_back_off_signature(backoff_key)) {
    if (auto fresh = fetch_server_keys_live(*origin, &db)) {
      db.add_signing_key(*origin, *fresh);
      keys = db.get_signing_keys(*origin);
    } else {
      db.back_off_signature(backoff_key);
    }
  }
  if (keys.empty()) {
