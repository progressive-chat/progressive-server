...
Ruma<RegisterRequest> Ruma<RegisterRequest>::from_request(const httplib::Request& req) {
  ...
  if (!req.body.empty()) {
    ...
    if (!body.is_discarded() && body.is_object()) {
      ...
      // NEW in 6e5b35ea: appservice bridges set from_appservice: true
...
Ruma<InviteRequest> Ruma<InviteRequest>::from_request(const httplib::Request& req) {
  ...
  if (!req.body.empty()) {
    ...
    if (!body.is_discarded() && body.is_object()) {
      ...
      // NEW in 346913268f: preserve an optional invite reason from the body.
      if (auto r = body.find("reason"); r != body.end() && r->is_string())
        wrapper.value.reason = r->get<std::string>();
    }
  }
  return wrapper;
}
