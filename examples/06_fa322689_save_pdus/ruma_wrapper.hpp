...
// NEW in fa322689: GET /_matrix/client/r0/sync (requires auth per METADATA).
struct SyncRequest {
  static constexpr bool REQUIRES_AUTH = true;
  // Query params (since, timeout, ...) ignored exactly as upstream did.
};

struct SyncResponse {
  std::string joined_room_id;
  std::vector<std::string> timeline_events;  // canonical PDU JSON strings
};
