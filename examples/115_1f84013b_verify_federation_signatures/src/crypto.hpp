...
// ruma_signatures::sign_json(hostname, keypair, json): adds
// signatures[hostname]["ed25519:<pub_b64>"] over canonical(json minus
// signatures). Used for key documents and federation requests (no content
// hash involved).
void sign_json(const std::string& hostname, const std::string& seed, json& obj);
