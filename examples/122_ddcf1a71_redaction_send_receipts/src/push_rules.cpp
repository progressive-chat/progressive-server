    ...
    if (condition.kind == "event_match") {
        ...
        if (event.contains("content") && event["content"].contains("body")) {
            try {
                ...
                // NEW in 662a0cf1: use the actual displayname (upstream
                // PushConditionRoomCtx.user_display_name); fall back to the
                // localpart when no displayname is known.
                std::string name = display_name;
                if (name.empty()) {
                    size_t colon = user_id.find(':');
                    name = (colon == std::string::npos) ? user_id
                        : user_id.substr(1, colon - 1);
                }
                if (!name.empty() && body.find(name) != std::string::npos) return true;
            } catch (...) {}
            return false;
        }
        return false;
...
PushRuleSet get_default_push_rules() {
    ...
    rules.content_rules.push_back(PushRule{
        ...
        {"notify"}, {{"highlight", true}, {"sound", "default"}}, true, 0});
    rules.content_rules.push_back(PushRule{
...
PushRuleResult evaluate_push_rules(
    ...
    for (const auto& rule : rules.global_override_rules) {
        ...
        for (const auto& cond : rule.conditions) {
            if (!evaluate_condition(cond, event, user_id, room_id, display_name)) {
                match = false;
                break;
            }
        }
...
PushActions get_actions(
    ...
    PushRuleResult r = evaluate_push_rules(rules, event, user_id, room_id, "", false, display_name);
    return PushActions{r.notify, r.highlight};
}
