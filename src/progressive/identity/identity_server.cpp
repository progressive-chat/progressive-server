...
// .well-known discovery, trusted server federation.
// Namespace: progressive::identity
...
class IdentityWellKnownManager;
...
bool valid_msisdn(const std::string& msisdn) {
  static const std::regex phone_re(R"(^\+\d{7,15}$)",
                                    std::regex::ECMAScript);
...
bool valid_server_name(const std::string& server_name) {
  static const std::regex sn_re(
      R"(^[a-zA-Z0-9]([a-zA-Z0-9\-.]*[a-zA-Z0-9])?(:\d{1,5})?$)");
  return std::regex_match(server_name, sn_re);
}
...
std::string sha256_hex_pepper(const std::string& pepper, const std::string& input) {
  return sha256_hex(pepper + "|" + input);
}
...
public:
  ...
  // Get validated session count
  size_t validated_session_count() {
    std::shared_lock<std::shared_mutex> lock(sessions_mutex_);
    size_t count = 0;
    for (const auto& [sid, session] : sessions_) {
      if (session.validated) count++;
    }
    return count;
  }
  ...
  // Get remaining rate limit budget for a given address
  int rate_limit_remaining(const std::string& medium, const std::string& address) {
    std::lock_guard<std::mutex> lock(ratelimit_mutex_);
    std::string key = medium + ":" + to_lower(address);
    int64_t now = now_ms();

    auto it = rate_limit_map_.find(key);
    if (it == rate_limit_map_.end()) return static_cast<int>(kMaxRequestsPerWindow);
    ...
    int used = static_cast<int>(entries.size());
    return static_cast<int>(kMaxRequestsPerWindow) - used;
  }

  // Export session list for admin purposes
  json export_sessions() {
    std::shared_lock<std::shared_mutex> lock(sessions_mutex_);
...
// Handles: bind, unbind, lookup operations with pepper-based hashing for privacy
...
public:
  ...
  ThreepidBinding bind_threepid(
    ...
    create_hash_entry(medium, address, mxid);
  ...
  // Bulk lookup: given a set of hash queries for multiple media, return all matches
  json bulk_lookup_by_hash(const json& query) {
    // query format: {"hashes": {"email": ["hash1","hash2"], "msisdn": ["hash3"]}}
    std::vector<std::pair<std::string, std::string>> queries;
    if (query.contains("hashes")) {
      for (auto& [medium, hash_list] : query["hashes"].items()) {
        if (hash_list.is_array()) {
          for (const auto& h : hash_list) {
            queries.emplace_back(medium, h.get<std::string>());
          }
        }
      }
    }
    auto results = lookup_by_hash(queries);
    json response;
  ...
  // Check if a specific mxid is bound to a 3pid
  bool is_bound_to_mxid(const std::string& medium,
                         const std::string& address,
                         const std::string& mxid) {
    std::shared_lock<std::shared_mutex> lock(mutex_);
    std::string key = to_lower(medium) + ":" + to_lower(address);
    auto it = bindings_.find(key);
    return it != bindings_.end() && it->second.mxid == mxid;
  }
  ...
  // Get hash lookup count
  size_t hash_count() {
    std::shared_lock<std::shared_mutex> lock(mutex_);
    return hash_lookups_.size();
  }
  ...
  // Lookup using old pepper (for transition period)
  std::optional<std::string> lookup_with_old_pepper(
      const std::string& old_pepper,
      const std::string& medium,
      const std::string& address_hash) {
    std::shared_lock<std::shared_mutex> lock(mutex_);
    std::string input = old_pepper + "|" + medium + "|" + address_hash;
    std::string old_style_hash = sha256_hex(input);
    auto it = hash_lookups_.find(old_style_hash);
    if (it != hash_lookups_.end()) return it->second.mxid;
    return std::nullopt;
  }

  // Get list of old peppers
  std::vector<std::string> get_old_peppers() const {
    std::shared_lock<std::shared_mutex> lock(mutex_);
    return old_peppers_;
  }
...
public:
  ...
  StoredInvite store_invite(
      ...
      const std::optional<std::string>& sender_display_name = std::nullopt,
      const std::optional<std::string>& sender_avatar_url = std::nullopt) {
    ...
    invite.sender_display_name = sender_display_name.value_or(sender);
    invite.sender_avatar_url = sender_avatar_url.value_or("");
    ...
    // Index by sender
    sender_invites_[sender].push_back(invite.invite_id);
  ...
  std::vector<StoredInvite> get_invites_from_sender(const std::string& sender) {
    std::shared_lock<std::shared_mutex> lock(mutex_);
    std::vector<StoredInvite> result;
    auto it = sender_invites_.find(sender);
    if (it != sender_invites_.end()) {
      for (const auto& invite_id : it->second) {
        auto inv_it = invites_.find(invite_id);
        if (inv_it != invites_.end()) {
          result.push_back(inv_it->second);
...
private:
  ...
  std::unordered_map<std::string, std::vector<std::string>> sender_invites_;
...
public:
  ...
  json get_policies(const std::string& language = "") {
    ...
    for (const auto& [key, policy] : policies_) {
      ...
      json version_obj = json::object();
      if (!language.empty()) {
        version_obj[policy.language] = json{
            {"name", policy.name},
            {"url", policy.url}
        };
      } else {
      ...
      grouped[policy.id][policy.version] = version_obj;
    }
  ...
  // Get all acceptance records for admin auditing
  json export_acceptances() {
    std::shared_lock<std::shared_mutex> lock(mutex_);
    json result = json::array();
    for (const auto& [user_id, user_policies] : acceptances_) {
      for (const auto& [policy_id, record] : user_policies) {
        result.push_back(json{
            {"user_id", record.user_id},
            {"policy_id", record.policy_id},
            {"policy_version", record.policy_version},
            {"accepted_by_ip", record.accepted_by_ip},
...
public:
  ...
  // Test SMTP connection (in production, would actually connect)
  json test_smtp_connection() {
    json result;
    result["configured"] = !config_.smtp_host.empty() && config_.smtp_port > 0;
    result["host"] = config_.smtp_host;
    result["port"] = config_.smtp_port;
    result["from"] = config_.from_address;
    // In production: would attempt actual connection and auth
    result["status"] = "ok";
    return result;
  }
...
public:
  ...
  json get_public_keys() {
    ...
    for (const auto& [key_id, key] : keys_) {
      ...
      key_json["expires_ts"] = key.expires_at_ms / 1000;
...
// ============================================================================
// IdentityWellKnownManager - .well-known discovery for identity servers
// Handles: GET /.well-known/matrix/identity
// Provides server discovery metadata for clients
// ============================================================================
class IdentityWellKnownManager {
public:
  struct DiscoveryInfo {
    std::string base_url;
    std::string identity_server_name;
    std::vector<std::string> supported_protocols;
    std::vector<std::string> supported_media;
    bool peering_enabled{true};
    bool invite_storage_enabled{true};
    int64_t last_updated_ms{0};
  };

  IdentityWellKnownManager() {
    discovery_.base_url = "https://matrix.local:8090";
    discovery_.identity_server_name = "matrix.local";
    discovery_.supported_protocols = {"v1", "v2"};
    discovery_.supported_media = {"email", "msisdn"};
    discovery_.last_updated_ms = now_ms();
  }

  // Set discovery info
  void set_discovery_info(const DiscoveryInfo& info) {
  ...
  void set_base_url(const std::string& url) {
    std::lock_guard<std::mutex> lock(mutex_);
    discovery_.base_url = url;
    discovery_.last_updated_ms = now_ms();
  }

  // Set identity server name
  void set_identity_server_name(const std::string& name) {
    std::lock_guard<std::mutex> lock(mutex_);
    discovery_.identity_server_name = name;
    discovery_.last_updated_ms = now_ms();
  }

  // Set supported protocols
  void set_supported_protocols(const std::vector<std::string>& protocols) {
    std::lock_guard<std::mutex> lock(mutex_);
    discovery_.supported_protocols = protocols;
    discovery_.last_updated_ms = now_ms();
  }

  // Set supported media
  void set_supported_media(const std::vector<std::string>& media) {
    std::lock_guard<std::mutex> lock(mutex_);
    discovery_.supported_media = media;
    discovery_.last_updated_ms = now_ms();
  }

  // Get the full well-known response
  json get_well_known_response() {
    std::lock_guard<std::mutex> lock(mutex_);
    json response;
    response["m.identity_server"] = json{
        {"base_url", discovery_.base_url},
        {"server_name", discovery_.identity_server_name}
    };
    response["m.identity_server"]["protocols"] = discovery_.supported_protocols;
    response["m.identity_server"]["media"] = discovery_.supported_media;
    response["m.identity_server"]["peering_enabled"] = discovery_.peering_enabled;
    response["m.identity_server"]["invite_storage_enabled"] =
        discovery_.invite_storage_enabled;
    return response;
  }

  // Get the identity server info JSON block
  json get_identity_server_info() {
    std::lock_guard<std::mutex> lock(mutex_);
    return json{
        {"base_url", discovery_.base_url},
        {"server_name", discovery_.identity_server_name},
        {"protocols", discovery_.supported_protocols},
        {"media", discovery_.supported_media},
        {"peering_enabled", discovery_.peering_enabled},
        {"invite_storage_enabled", discovery_.invite_storage_enabled}
    };
  }

  // Check if a given medium is supported
  bool is_medium_supported(const std::string& medium) {
    std::lock_guard<std::mutex> lock(mutex_);
    return std::find(discovery_.supported_media.begin(),
                     discovery_.supported_media.end(),
                     medium) != discovery_.supported_media.end();
  }

  // Check if a protocol version is supported
  bool is_protocol_supported(const std::string& protocol) {
    std::lock_guard<std::mutex> lock(mutex_);
    return std::find(discovery_.supported_protocols.begin(),
                     discovery_.supported_protocols.end(),
                     protocol) != discovery_.supported_protocols.end();
  }

  // Get discovery info
  DiscoveryInfo get_discovery_info() const {
    std::lock_guard<std::mutex> lock(mutex_);
    return discovery_;
  }
...
private:
  ...
  DiscoveryInfo discovery_;
};

// ============================================================================
// IdentityFederationManager - trusted server federation
// Handles: trusted server list, peering, cross-server lookup
// Equivalent to synapse.federation.transport.client identity portions
// ============================================================================
class IdentityFederationManager {
public:
  struct TrustedServer {
    ...
    int64_t trust_level{0};         // 0 = untrusted, 1 = trusted, 2 = fully trusted
    bool enabled{true};
    bool peering_enabled{true};
    json metadata;
  };
  ...
  IdentityFederationManager(const std::string& own_server_name)
      : own_server_name_(own_server_name) {}

  // Add a trusted server
  TrustedServer add_trusted_server(
      const std::string& server_name,
      const std::string& base_url,
      const std::string& public_key = "",
      int64_t trust_level = 1) {

    TrustedServer server;
    server.server_name = server_name;
    server.base_url = base_url;
    server.public_key = public_key;
    server.key_id = generate_token(16);
    server.added_at_ms = now_ms();
    server.last_seen_ms = now_ms();
    server.trust_level = trust_level;
    server.enabled = true;
    server.peering_enabled = true;

    std::lock_guard<std::shared_mutex> lock(mutex_);
    trusted_servers_[server_name] = server;
    return server;
  }

  // Remove a trusted server
  bool remove_trusted_server(const std::string& server_name) {
    std::lock_guard<std::shared_mutex> lock(mutex_);
    return trusted_servers_.erase(server_name) > 0;
  }

  // Get a trusted server by name
  std::optional<TrustedServer> get_trusted_server(const std::string& server_name) {
    std::shared_lock<std::shared_mutex> lock(mutex_);
    auto it = trusted_servers_.find(server_name);
    if (it != trusted_servers_.end()) return it->second;
    return std::nullopt;
  }

  // Check if a server is trusted
  bool is_trusted(const std::string& server_name, int min_trust_level = 1) {
    std::shared_lock<std::shared_mutex> lock(mutex_);
    auto it = trusted_servers_.find(server_name);
    if (it == trusted_servers_.end()) return false;
    return it->second.enabled && it->second.trust_level >= min_trust_level;
  }

  // Get list of all trusted servers
  std::vector<TrustedServer> get_trusted_servers(int min_trust_level = 1) {
    std::shared_lock<std::shared_mutex> lock(mutex_);
    std::vector<TrustedServer> result;
    for (const auto& [name, server] : trusted_servers_) {
      if (server.enabled && server.trust_level >= min_trust_level) {
  ...
  void mark_seen(const std::string& server_name) {
    std::lock_guard<std::shared_mutex> lock(mutex_);
    auto it = trusted_servers_.find(server_name);
    if (it != trusted_servers_.end()) {
      it->second.last_seen_ms = now_ms();
    }
  }

  // Disable peering with a server
  void disable_peering(const std::string& server_name) {
    std::lock_guard<std::shared_mutex> lock(mutex_);
    auto it = trusted_servers_.find(server_name);
    if (it != trusted_servers_.end()) {
      it->second.peering_enabled = false;
    }
  }

  // Enable peering with a server
  void enable_peering(const std::string& server_name) {
    std::lock_guard<std::shared_mutex> lock(mutex_);
    auto it = trusted_servers_.find(server_name);
    if (it != trusted_servers_.end()) {
      it->second.peering_enabled = true;
    }
  }

  // Cross-server lookup: query trusted servers for 3PID hashes
  json cross_server_lookup(
      const std::string& medium,
      const std::vector<std::string>& address_hashes) {
    ...
    std::shared_lock<std::shared_mutex> lock(mutex_);
    for (const auto& [name, server] : trusted_servers_) {
      if (!server.enabled || !server.peering_enabled) continue;
      if (server.trust_level < 1) continue;

      // In production, this would make HTTP calls to the peer identity server.
      // For the massive handler, we simulate the federation response.
      json peer_result;
      peer_result["server_name"] = name;
      peer_result["results"] = json::array();
      for (const auto& hash : address_hashes) {
        peer_result["results"].push_back(json{
            {"hash", hash},
            {"server", name}
        });
      }
      result["mappings"].push_back(peer_result);
    }
    return result;
  }

  // Generate a peering invitation request
  json generate_peering_request(const std::string& target_server) {
    json request;
    request["method"] = "m.identity.peering_request";
    request["origin"] = own_server_name_;
    request["destination"] = target_server;
    request["timestamp_ms"] = now_ms();
    request["request_id"] = generate_token(24);
    request["supported_media"] = json::array({"email", "msisdn"});
    request["supported_protocols"] = json::array({"v1", "v2"});
    return request;
  }

  // Process a peering request from another server
  json process_peering_request(const json& request) {
    std::string origin = request.value("origin", "");
    ...
    if (origin.empty()) {
      response["errcode"] = "M_MISSING_PARAM";
      response["error"] = "Missing origin";
      return response;
    }

    // Check if we trust this origin
    auto server = get_trusted_server(origin);
    if (!server.has_value()) {
      response["errcode"] = "M_UNKNOWN_SERVER";
      response["error"] = "Server not trusted: " + origin;
      response["accepted"] = false;
      return response;
    }

    response["accepted"] = true;
    response["server_name"] = own_server_name_;
    response["trust_level"] = server->trust_level;
  ...
  json get_stats() {
    std::shared_lock<std::shared_mutex> lock(mutex_);
    size_t total = trusted_servers_.size();
    size_t enabled = 0, peering = 0, fully_trusted = 0;
    for (const auto& [name, server] : trusted_servers_) {
      if (server.enabled) enabled++;
      if (server.peering_enabled) peering++;
      if (server.trust_level >= 2) fully_trusted++;
    }
    return json{
        {"own_server_name", own_server_name_},
        {"total_trusted_servers", total},
        {"enabled_servers", enabled},
        {"peering_enabled_count", peering},
        {"fully_trusted_count", fully_trusted}
    };
  }

  // Export trusted server list
  json export_trusted_servers() {
    std::shared_lock<std::shared_mutex> lock(mutex_);
    json result = json::array();
    for (const auto& [name, server] : trusted_servers_) {
      result.push_back(json{
          ...
          {"peering_enabled", server.peering_enabled},
          {"added_at_ms", server.added_at_ms},
          {"last_seen_ms", server.last_seen_ms}
      });
    }
    return result;
  }

  // Import trusted servers
  void import_trusted_servers(const json& data) {
    std::lock_guard<std::shared_mutex> lock(mutex_);
    for (const auto& item : data) {
      TrustedServer server;
      server.server_name = item.value("server_name", "");
      server.base_url = item.value("base_url", "");
      server.public_key = item.value("public_key", "");
      server.key_id = item.value("key_id", generate_token(16));
      server.trust_level = item.value("trust_level", 1);
      server.enabled = item.value("enabled", true);
      server.peering_enabled = item.value("peering_enabled", true);
      server.added_at_ms = item.value("added_at_ms", now_ms());
      server.last_seen_ms = now_ms();
      trusted_servers_[server.server_name] = server;
    }
  }

  // Set own server name
  void set_own_server_name(const std::string& name) {
    own_server_name_ = name;
  }

  // Get own server name
  std::string get_own_server_name() const { return own_server_name_; }

private:
  std::string own_server_name_;
  std::shared_mutex mutex_;
  std::unordered_map<std::string, TrustedServer> trusted_servers_;
};
...
public:
  struct IdentityConfig {
    ...
    std::string base_url{"https://matrix.local:8090"};
  };

  IdentityServer()
      : pubkey_manager_("localhost"),
        federation_manager_("localhost") {
    // Initialize with default trusted servers if configured
    wellknown_.set_base_url(config_.base_url);
    wellknown_.set_identity_server_name(config_.server_name);
  }
  ...
  explicit IdentityServer(const IdentityConfig& config)
      : config_(config),
        ...
        federation_manager_(config.server_name) {
    wellknown_.set_base_url(config.base_url);
    wellknown_.set_identity_server_name(config.server_name);

    // Seed federation with configured trusted servers
    if (!config.trusted_id_servers.empty()) {
      auto servers = split_string(config.trusted_id_servers, ',');
      for (const auto& srv : servers) {
        std::string trimmed = srv;
        trimmed.erase(0, trimmed.find_first_not_of(" \t"));
        trimmed.erase(trimmed.find_last_not_of(" \t") + 1);
        if (!trimmed.empty()) {
          federation_manager_.add_trusted_server(trimmed,
              "https://" + trimmed, "", 1);
        }
      }
    }
  ...
  void set_config(const IdentityConfig& config) {
    ...
    wellknown_.set_base_url(config.base_url);
    wellknown_.set_identity_server_name(config.server_name);
    federation_manager_.set_own_server_name(config.server_name);
  ...
  IdentityWellKnownManager& wellknown() { return wellknown_; }
  IdentityFederationManager& federation() { return federation_manager_; }
  ...
  // POST /lookup - privacy-preserving lookup (hash-based)
  ...
  // POST /bulk_lookup - Bulk hash-based lookup across media
  json handle_bulk_lookup(const json& body) {
    // Body: {"threepids": [["email", "hash1"], ["msisdn", "hash2"], ...]}
    // Or body with pepper and algorithm
    std::string pepper = body.value("pepper", "");
    std::string algorithm = body.value("algorithm", "sha256");

    std::vector<std::pair<std::string, std::string>> queries;

    if (body.contains("threepids") && body["threepids"].is_array()) {
      for (const auto& threepid : body["threepids"]) {
        if (threepid.is_array() && threepid.size() >= 2) {
          std::string medium = threepid[0].get<std::string>();
          std::string address = threepid[1].get<std::string>();
          // If pepper provided, rehash the address with it
          if (!pepper.empty()) {
            std::string hashed = sha256_hex_pepper(pepper,
                to_lower(medium) + "|" + to_lower(address));
            queries.emplace_back(medium, hashed);
          } else {
            queries.emplace_back(medium, address);
          }
        }
      }
    } else if (body.contains("hashes")) {
      // Same format as lookup
      for (auto& [medium, hash_list] : body["hashes"].items()) {
        if (hash_list.is_array()) {
          for (const auto& h : hash_list) {
            queries.emplace_back(medium, h.get<std::string>());
          }
      ...
      return json{{"errcode", "M_MISSING_PARAM"},
                  {"error", "Missing threepids or hashes"}};
    }
    ...
    // Also query trusted federation servers if federation enabled
    if (config_.enable_federation && response["mappings"].empty()) {
      // Gather hashes from queries for cross-server lookup
      std::vector<std::string> hashes_to_query;
      for (const auto& [medium, hash] : queries) {
        hashes_to_query.push_back(hash);
      }
      // Try cross-server lookup for the first medium
      if (!queries.empty()) {
        response["federation_mappings"] =
            federation_manager_.cross_server_lookup(queries[0].first, hashes_to_query);
      }
    }
  ...
  json handle_store_invite(const json& body) {
    ...
    std::string sender_display_name = body.value("sender_display_name", "");
    std::string sender_avatar_url = body.value("sender_avatar_url", "");
    ...
    auto invite = invite_manager_.store_invite(
        ...
        public_keys,
        sender_display_name.empty() ? std::nullopt
                                    : std::optional<std::string>(sender_display_name),
        sender_avatar_url.empty() ? std::nullopt
                                  : std::optional<std::string>(sender_avatar_url));
  ...
  json handle_accept_terms(const json& body, const std::string& ip) {
    ...
    for (const auto& policy_ref : accepted) {
      ...
      // Try to parse the format: "policy_id/version/language" or just "policy_id"
  ...
  // GET /.well-known/matrix/identity
  json handle_well_known() {
    if (!config_.enable_well_known) {
      return json{{"errcode", "M_NOT_FOUND"}, {"error", "Well-known disabled"}};
    }
    return wellknown_.get_well_known_response();
  }

  // POST /_matrix/identity/federation/peering_request
  json handle_peering_request(const json& body) {
    if (!config_.enable_federation) {
      return json{{"errcode", "M_FORBIDDEN"},
                  {"error", "Federation is disabled"}};
    }
    return federation_manager_.process_peering_request(body);
  }

  // GET /_matrix/identity/federation/servers
  json handle_federation_servers() {
    if (!config_.enable_federation) {
      return json{{"errcode", "M_FORBIDDEN"},
                  {"error", "Federation is disabled"}};
    }
    return federation_manager_.export_trusted_servers();
  }

  // POST /_matrix/identity/federation/servers
  json handle_federation_servers_add(const json& body) {
    if (!config_.enable_federation) {
      return json{{"errcode", "M_FORBIDDEN"},
                  {"error", "Federation is disabled"}};
    }

    std::string server_name = body.value("server_name", "");
    std::string base_url = body.value("base_url", "");
    int64_t trust_level = body.value("trust_level", 1);

    if (server_name.empty() || base_url.empty()) {
      return json{{"errcode", "M_MISSING_PARAM"},
                  {"error", "Missing server_name or base_url"}};
    }

    auto server = federation_manager_.add_trusted_server(
        server_name, base_url, "", trust_level);
    return json{
        {"server_name", server.server_name},
        {"trust_level", server.trust_level},
        {"added", true}
  ...
  json get_server_stats() {
    json stats = json{
        ...
        {"validated_sessions", session_manager_.validated_session_count()},
        {"bindings", binder_.binding_count()},
        {"hash_lookups", binder_.hash_count()},
    ...
    stats["federation_stats"] = federation_manager_.get_stats();
  ...
  // Accept a stored invite (resolve the 3PID to mxid)
  json handle_accept_invite(const std::string& invite_id, const std::string& mxid) {
    bool success = invite_manager_.accept_invite(invite_id, mxid);
    if (!success) {
      return json{{"errcode", "M_NOT_FOUND"},
                  {"error", "Invite not found or already processed"}};
    }
    return json{{"success", true}, {"invite_id", invite_id}, {"mxid", mxid}};
  }

  // Reject a stored invite
  json handle_reject_invite(const std::string& invite_id) {
    bool success = invite_manager_.reject_invite(invite_id);
    if (!success) {
      return json{{"errcode", "M_NOT_FOUND"},
                  {"error", "Invite not found or already processed"}};
    }
    return json{{"success", true}, {"invite_id", invite_id}};
  }
...
private:
  ...
  IdentityWellKnownManager wellknown_;
  IdentityFederationManager federation_manager_;
};
...
// --- BulkLookupServlet ---
class BulkLookupServlet : public rest::BaseRestServlet {
public:
  explicit BulkLookupServlet(IdentityServer& server) : server_(server) {}

  std::vector<std::string> patterns() const override {
    return {R"(^/bulk_lookup$)", R"(^/bulkLookup$)"};
  }
  ...
  rest::HttpResponse on_request(const rest::HttpRequest& req) override {
    ...
    try {
      json body = json::parse(req.body.empty() ? "{}" : req.body);
      json result = server_.handle_bulk_lookup(body);
      if (result.contains("errcode")) {
        return rest::BaseRestServlet::error_response(
            400, result["errcode"], result["error"]);
      }
...
// --- WellKnownServlet ---
class WellKnownServlet : public rest::BaseRestServlet {
public:
  explicit WellKnownServlet(IdentityServer& server) : server_(server) {}

  std::vector<std::string> patterns() const override {
    return {R"(^/\.well-known/matrix/identity$)"};
  }
  ...
  rest::HttpResponse on_request(const rest::HttpRequest& req) override {
    ...
    json result = server_.handle_well_known();
    if (result.contains("errcode")) {
      return rest::BaseRestServlet::error_response(
          404, result["errcode"], result["error"]);
    }
...
// --- FederationServersServlet ---
class FederationServersServlet : public rest::BaseRestServlet {
public:
  explicit FederationServersServlet(IdentityServer& server) : server_(server) {}

  std::vector<std::string> patterns() const override {
    return {R"(^/_matrix/identity/federation/servers$)"};
  }
  ...
  rest::HttpResponse on_request(const rest::HttpRequest& req) override {
    ...
    if (req.method == "POST") {
      try {
        json body = json::parse(req.body.empty() ? "{}" : req.body);
        json result = server_.handle_federation_servers_add(body);
        if (result.contains("errcode")) {
          return rest::BaseRestServlet::error_response(
              400, result["errcode"], result["error"]);
        }
    ...
    // GET: list trusted servers
    json result = server_.handle_federation_servers();
    if (result.contains("errcode")) {
      return rest::BaseRestServlet::error_response(
          403, result["errcode"], result["error"]);
    }
    return rest::BaseRestServlet::success_response(result);
  }
...
// --- PeeringRequestServlet ---
class PeeringRequestServlet : public rest::BaseRestServlet {
public:
  explicit PeeringRequestServlet(IdentityServer& server) : server_(server) {}

  std::vector<std::string> patterns() const override {
    return {R"(^/_matrix/identity/federation/peering_request$)"};
  }
  ...
  rest::HttpResponse on_request(const rest::HttpRequest& req) override {
    ...
    try {
      json body = json::parse(req.body.empty() ? "{}" : req.body);
      json result = server_.handle_peering_request(body);
      if (result.contains("errcode")) {
        return rest::BaseRestServlet::error_response(
            400, result["errcode"], result["error"]);
      }
...
// --- IdentityAcceptInviteServlet ---
class IdentityAcceptInviteServlet : public rest::BaseRestServlet {
public:
  explicit IdentityAcceptInviteServlet(IdentityServer& server) : server_(server) {}

  std::vector<std::string> patterns() const override {
    return {R"(^/_matrix/identity/admin/invites/([a-zA-Z0-9_]+)/accept$)"};
  }
  ...
  rest::HttpResponse on_request(const rest::HttpRequest& req) override {
    ...
    try {
      std::string invite_id;
      auto id_it = req.path_params.find("1");
      if (id_it != req.path_params.end()) invite_id = id_it->second;

      json body = json::parse(req.body.empty() ? "{}" : req.body);
      std::string mxid = body.value("mxid", "");

      if (invite_id.empty() || mxid.empty()) {
        return rest::BaseRestServlet::error_response(
            400, "M_MISSING_PARAM", "Missing invite_id or mxid");
      }

      json result = server_.handle_accept_invite(invite_id, mxid);
      if (result.contains("errcode")) {
        return rest::BaseRestServlet::error_response(
            400, result["errcode"], result["error"]);
      }
...
// --- IdentityRejectInviteServlet ---
class IdentityRejectInviteServlet : public rest::BaseRestServlet {
public:
  explicit IdentityRejectInviteServlet(IdentityServer& server) : server_(server) {}

  std::vector<std::string> patterns() const override {
    return {R"(^/_matrix/identity/admin/invites/([a-zA-Z0-9_]+)/reject$)"};
  }
  ...
  rest::HttpResponse on_request(const rest::HttpRequest& req) override {
    ...
    if (invite_id.empty()) {
      return rest::BaseRestServlet::error_response(
          400, "M_MISSING_PARAM", "Missing invite_id");
    }

    json result = server_.handle_reject_invite(invite_id);
    if (result.contains("errcode")) {
      return rest::BaseRestServlet::error_response(
          400, result["errcode"], result["error"]);
    }
...
// --- IdentitySessionListServlet ---
class IdentitySessionListServlet : public rest::BaseRestServlet {
public:
  explicit IdentitySessionListServlet(IdentityServer& server) : server_(server) {}

  std::vector<std::string> patterns() const override {
    return {R"(^/_matrix/identity/admin/sessions$)"};
  }
  ...
  rest::HttpResponse on_request(const rest::HttpRequest& req) override {
    ...
    json result = server_.sessions().export_sessions();
    return rest::BaseRestServlet::success_response(result);
  }
...
void register_identity_servlets(rest::ServletRegistry& registry,
  ...
  registry.register_servlet(
      std::make_unique<BulkLookupServlet>(server));
  registry.register_servlet(
  ...
  registry.register_servlet(
      std::make_unique<WellKnownServlet>(server));
  registry.register_servlet(
      std::make_unique<FederationServersServlet>(server));
  registry.register_servlet(
      std::make_unique<PeeringRequestServlet>(server));
  registry.register_servlet(
  ...
  registry.register_servlet(
      std::make_unique<IdentityAcceptInviteServlet>(server));
  registry.register_servlet(
      std::make_unique<IdentityRejectInviteServlet>(server));
  registry.register_servlet(
      std::make_unique<IdentitySessionListServlet>(server));
}
