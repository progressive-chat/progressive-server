...
ruma::MatrixResult<ruma::CreateRoomResponse> create_room_route(
  ...
  // NEW in b5e3185 (MSC4289): m.room.create content with additional_creators and room_version
  ...
  const std::string join_rule = (body.preset.value_or("private_chat") == "public_chat") ? "public" : "invite";
  ...
  // m.room.create must be the first event (auth: prev_events empty).
  append_state("m.room.create", json{{"creator", creator}}, "");
