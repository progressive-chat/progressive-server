...
int main() {
  ...
  // ---- NEW in 1af6dd98: server publishes signed keys; verify PDU EXTERNALLY --
  {
    auto keys = client.Get("/_matrix/key/v2/server");
    bool doc_ok = false, pdu_ok = false;
    std::string pub_b64;
    if (keys && keys->status == 200) {
      auto doc = nlohmann::json::parse(keys->body, nullptr, false);
      if (!doc.is_discarded() && doc.contains("verify_keys")) {
        for (auto& [kid, entry] : doc["verify_keys"].items()) {
          pub_b64 = entry.value("key", "");
          // The key document itself must carry a valid self-signature.
          nlohmann::json to_check = doc;
          to_check.erase("signatures");
          const std::string server_name = doc.value("server_name", "");
          doc_ok = crypto::ed25519_verify(
              pub_b64, to_check.dump(),
              doc["signatures"][server_name].value(kid, ""));
        }
      }
    }
    expect(doc_ok, "key document self-signature verifies");

    // Pull a synced event and verify it against the PUBLISHED key — the same
    // flow a federating homeserver would use (fetch keys, then verify).
    r = client.Get("/_matrix/client/r0/sync",
                   {{"Authorization", "Bearer " + access_token}});
    if (r && r->status == 200) {
      auto body = nlohmann::json::parse(r->body, nullptr, false);
      for (auto& [rid, room] : body["rooms"]["join"].items())
        for (auto& ev : room["timeline"]["events"]) {
          ...
          to_verify.erase("unsigned");
          if (!pub_b64.empty() &&
              crypto::ed25519_verify(pub_b64, to_verify.dump(), sig)) {
            pdu_ok = true;
            break;
          }
        }
    }
    expect(pdu_ok, "event signature verifies against published key");
  }
