...
// NEW in dcac1361: event search across multiple rooms.
// POST /_matrix/client/r0/search with search_categories.room_events.
struct SearchEventsRequest {
  static constexpr bool REQUIRES_AUTH = true;
  struct Filter {
    std::optional<std::vector<std::string>> rooms;  // nullopt = all joined
  ...
  struct SearchCategories {
    std::optional<RoomEvents> room_events;
  };
  std::optional<SearchCategories> search_categories;
  std::optional<std::string> next_batch;  // pagination offset token
};

// NEW in dcac1361: spec-shaped response payload
// (search_categories.room_events.{count,highlights,next_batch,results}).
struct SearchEventsResponse {
  int64_t count = 0;
  std::vector<std::string> highlights;
  std::string next_batch;  // empty = no further pages
  // Each entry: {"rank": double, "result": <event object>}.
