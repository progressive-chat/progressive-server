...
class Data {
 public:
  ...
  bool pdu_append(const std::string& event_id, const std::string& room_id,
                  nlohmann::json event);
  // NEW in c4f5a0a6: generate a new StateHash for an incoming state PDU.
  // Returns the hash and updates the stateid_pduid / pduid_statehash trees.
  std::string append_state_pdu(const std::string& room_id,
                               const std::string& pdu_id,
                               const std::string& state_key,
                               const std::string& event_type);
  // NEW in c4f5a0a6: compute a new StateHash from a set of state PDU ids.
  std::string new_state_hash_id(const std::string& room_id);
  // NEW in c4f5a0a6: get the current state of a room (all (type,key) -> pdu_id).
