...
// NEW in c3fb1b0: a single media retention policy ("scope"). Fields map to
// Conduit's `retention` config items: `accessed` (last-access age, ms),
// `created` (age, ms) and `space` (bytes). A null `scope` is the default and
// applies to media not covered by a more specific scope.
struct RetentionPolicy {
  std::optional<std::string> scope;  // "local", "remote", "thumbnail" or nullopt (default)
  std::optional<uint64_t> accessed_ms;
  std::optional<uint64_t> created_ms;
  std::optional<uint64_t> space_bytes;
};
...
class Media {
 public:
  ...
  struct FileInfo {
    uint64_t creation = 0;      // unix seconds
    uint64_t last_access = 0;   // unix seconds
    uint64_t size = 0;          // bytes
  };
  struct MediaQueryFileInfo {
    std::optional<std::string> uploader;
    std::string sha256_hex;
    std::optional<std::string> filename;
    std::optional<std::string> content_type;
    bool unauthenticated_access_permitted = false;
    bool is_blocked_via_filehash = false;
    FileInfo file_info;
  };
  struct MediaQuery {
    bool is_blocked = false;
    std::optional<MediaQueryFileInfo> source_file;
  };
  ...
  // Full information about one media item (used by the admin `query` command).
  std::optional<MediaQuery> media_query(const std::string& server_name,
                                        const std::string& media_id) const;
  // List stored media with optional filters: `server_name` (only media on that
  // server), `user_id` (only media uploaded by that user), `content_type`
  // (exact), `before_ms`/`after_ms` (creation window), and `include_thumbs`
  // (we store no thumbnails, so it is vacuous).
  std::vector<MediaListItem> media_list(const std::optional<std::string>& server_name,
                                        const std::optional<std::string>& user_id,
  ...
  // Update `last_access` (ms) on a media entry (called on every download and
  // thumbnail generation), so the `accessed` retention can evict unused media.
  void update_last_access(const std::string& server_name, const std::string& media_id);
  // Deletes all media that exceed the `accessed`/`created` requirements of the
  // policies that apply to them. Returns the number of media removed.
  size_t cleanup_time_retention(const std::vector<RetentionPolicy>& policies);
  // If the `space` requirement would be exceeded by storing `new_size` bytes of
  // the given media class (is_thumb=false: local/remote files; true: thumbnails),
  // evict least-recently-accessed media until there is enough space. Returns the
  // number of media evicted.
  size_t clear_required_space(const std::vector<RetentionPolicy>& policies,
